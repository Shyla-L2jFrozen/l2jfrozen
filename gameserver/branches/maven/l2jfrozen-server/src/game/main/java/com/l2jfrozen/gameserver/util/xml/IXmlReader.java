...
{
	static final Logger LOG = Logger.getLogger(IXmlReader.class);
	...
	default void parseFile(File f)
	{
		...
		{
			LOG.warn("Could not parse {"+f.getName()+"} is not a file or it doesn't exist!" );
			return;
		}
		...
		{
			LOG.warn("Could not parse file {"+f.getName()+"} at line {"+e.getLineNumber()+"}, column {"+e.getColumnNumber()+"}", e);
			return;
		}
		catch (Exception e)
		{
			LOG.warn("Could not parse file {"+f.getName()+"}", e);
			return;
		}
	}
	...
	default boolean parseDirectory(File dir, boolean recursive)
	{
		if (!dir.exists())
		{
			LOG.warn("Folder {"+dir.getAbsolutePath()+"} doesn't exist!");
			return false;
		}
	...
	{
		LOG.error("Parser not implemented!");
	}
	...
	default <T extends Enum<T>> T parseEnum(Node node, Class<T> clazz, T defaultValue)
	{
		...
		{
			LOG.warn("Invalid value specified for node: {"+node.getNodeName()+"} specified value: {"+node.getNodeValue()+"} should be enum value of \"{"+clazz.getSimpleName()+"}\" using default value: {"+defaultValue+"}");
			return defaultValue;
		}
	}
