package com.l2jfrozen.common.util.database;
...
public class L2DatabaseFactory_c3p0 extends L2DatabaseFactory
{
	...
	public L2DatabaseFactory_c3p0()
	{
		try
		{
			if (CommonConfig.DATABASE_MAX_CONNECTIONS < 2)
			{
				CommonConfig.DATABASE_MAX_CONNECTIONS = 2;
				LOGGER.warn("A minimum of " + CommonConfig.DATABASE_MAX_CONNECTIONS + " db connections are required.");
			}
			...
			_source.setMaxPoolSize(Math.max(10, CommonConfig.DATABASE_MAX_CONNECTIONS));
			...
			_source.setMaxIdleTime(CommonConfig.DATABASE_MAX_IDLE_TIME); // 0 = idle connections never expire
			...
			_source.setUser(CommonConfig.DATABASE_LOGIN);
