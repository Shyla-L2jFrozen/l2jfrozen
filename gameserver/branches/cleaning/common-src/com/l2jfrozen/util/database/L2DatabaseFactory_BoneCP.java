...
public class L2DatabaseFactory_BoneCP extends L2DatabaseFactory
{
	...
	{
		LOGGER.info("Initializing BoneCP [ databaseDriver -> " + CommonConfig.DATABASE_DRIVER + ", jdbcUrl -> " + CommonConfig.DATABASE_URL + ", maxConnectionsPerPartition -> " + CommonConfig.DATABASE_MAX_CONNECTIONS + ", username -> " + CommonConfig.DATABASE_LOGIN + ", password -> " + CommonConfig.DATABASE_PASSWORD + " ]");
		
		try
		{
			if (CommonConfig.DATABASE_MAX_CONNECTIONS < 10)
			{
				CommonConfig.DATABASE_MAX_CONNECTIONS = 10;
				LOGGER.warn("at least " + CommonConfig.DATABASE_MAX_CONNECTIONS + " db connections are required.");
			}
			
			if (CommonConfig.DATABASE_PARTITION_COUNT > 4)
			{
				CommonConfig.DATABASE_PARTITION_COUNT = 4;
				LOGGER.warn("max {} db connections partitions. " + CommonConfig.DATABASE_PARTITION_COUNT);
			}
			
			if (CommonConfig.DATABASE_MAX_CONNECTIONS * CommonConfig.DATABASE_PARTITION_COUNT > 200)
			{
				LOGGER.warn("Max Connections number is higher then 60.. Using Partition 2 and Connection 30");
				CommonConfig.DATABASE_MAX_CONNECTIONS = 50;
				CommonConfig.DATABASE_PARTITION_COUNT = 4;
			}
			...
			_source.getConfig().setMaxConnectionsPerPartition(CommonConfig.DATABASE_MAX_CONNECTIONS);
			
			_source.getConfig().setPartitionCount(CommonConfig.DATABASE_PARTITION_COUNT);
			...
			_source.setConnectionTimeoutInMs(CommonConfig.DATABASE_TIMEOUT);
			...
			_source.setDriverClass(CommonConfig.DATABASE_DRIVER);
			_source.setJdbcUrl(CommonConfig.DATABASE_URL);
			_source.setUsername(CommonConfig.DATABASE_LOGIN);
			_source.setPassword(CommonConfig.DATABASE_PASSWORD);
	...
	public Connection getConnection(final boolean checkclose)
	{
		...
		while (con == null)
		{
			try
			{
				...
				if (checkclose)
					ThreadPoolManager.getInstance().scheduleGeneral(new ConnectionCloser(con, new RuntimeException()), CommonConfig.DATABASE_CONNECTION_TIMEOUT);
			}
