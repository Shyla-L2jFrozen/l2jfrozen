...
public class Shutdown extends Thread
{
	...
	{
		/*
		 * // disallow new logins try { //Doesnt actually do anything //Server.gameServer.getLoginController().setMaxAllowedOnlinePlayers(0); } catch(Throwable t) { if(CommonConfig.ENABLE_ALL_EXCEPTIONS) t.printStackTrace(); }
		 */
	...
	private void disconnectAllCharacters()
	{
		...
		for (final L2PcInstance player : L2World.getInstance().getAllPlayers())
		{
			...
			try
			{
				...
				{
					client.sendPacket(ServerClose.STATIC_PACKET);
					client.close(0);
