...
public class AdminBan implements IAdminCommandHandler
{
	...
	private void auditAction(final String fullCommand, final L2PcInstance activeChar, final String target)
	{
		...
		GMAudit.auditGMAction(activeChar.getName() + " [" + activeChar.getObjectId() + "]", activeChar.getClient().getConnection().c().getHostAddress(), command[0], (target.equals("") ? "no-target" : target), (command.length > 2 ? command[2] : ""));
	}
