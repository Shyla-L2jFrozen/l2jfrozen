...
public class AdminBan implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(final String command, final L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_ban_acc"))
		{
			...
			{
				targetPlayer.setPunishLevel(PunishLevel.ACC, 0);
		...
		else if (command.startsWith("admin_banchat"))
		{
			...
			if (targetPlayer != null)
			{
				...
				targetPlayer.setPunishLevel(PunishLevel.CHAT, duration);
				if (duration > 0)
		...
		else if (command.startsWith("admin_unbanchat"))
		{
			...
			if (targetPlayer != null)
			{
				...
				{
					targetPlayer.setPunishLevel(PunishLevel.NONE, 0);
		...
		else if (command.startsWith("admin_jail"))
		{
			...
			if (targetPlayer != null)
			{
				targetPlayer.setPunishLevel(PunishLevel.JAIL, duration);
	...
	private void auditAction(final String fullCommand, final L2PcInstance activeChar, final String target)
	{
		...
		GMAudit.auditGMAction(activeChar.getName() + " [" + activeChar.getObjectId() + "]", activeChar.getClient().gco().getInetAddress().getHostAddress(), command[0], (target.equals("") ? "no-target" : target), (command.length > 2 ? command[2] : ""));
	}
	...
	private void banChatOfflinePlayer(final L2PcInstance activeChar, final String name, final int delay, final boolean ban)
	{
		...
		if (ban)
		{
			level = PunishLevel.CHAT.value();
			value = (delay > 0 ? delay * 60000L : 60000);
		}
		else
		{
			level = PunishLevel.NONE.value();
			value = 0;
		}
	...
	private void jailOfflinePlayer(final L2PcInstance activeChar, final String name, final int delay)
	{
		...
		try
		{
			...
			statement.setInt(4, PunishLevel.JAIL.value());
