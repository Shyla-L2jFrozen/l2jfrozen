...
public final class L2PcInstance extends L2PlayableInstance
{
	...
	private boolean checkAntiFarm(final L2PcInstance targetPlayer)
	{
		...
		if (Config.ANTI_FARM_ENABLED)
		{
			...
			if (Config.ANTI_FARM_IP_ENABLED)
			{
				...
				{
					final String ip1 = this.getClient().gco().getInetAddress().getHostAddress();
					final String ip2 = targetPlayer.getClient().gco().getInetAddress().getHostAddress();
	...
	private void addItemReward(final L2PcInstance targetPlayer)
	{
		...
		{
			if (targetPlayer.getClient().gco().getInetAddress() != getClient().gco().getInetAddress())
			{
	...
	public synchronized void deleteMe(final boolean disconnect)
	{
		if (Config.DEVELOPER)
		{
			if (getClient() != null && getClient().gco() != null && getClient().gco().getInetAddress().getHostAddress() != null)
				LOGGER.info("Removing character: [" + getName() + "], account: [" + getAccountName() + "], ip: [" + getClient().gco().getInetAddress().getHostAddress() + "]");
			else
	...
	public boolean checkMultiBox()
	{
		...
		if (getClient() != null && getClient().gco() != null && !getClient().gco().isClosed() && getClient().gco().getInetAddress() != null)
		{
			
			final String thisip = getClient().gco().getInetAddress().getHostAddress();
			final Collection<L2PcInstance> allPlayers = L2World.getInstance().getAllPlayers();
			for (final L2PcInstance player : allPlayers)
			{
				if (player != null)
				{
					if (player.isOnline() == 1 && player.getClient() != null && player.getClient().gco() != null && !player.getClient().gco().isClosed() && player.getClient().gco().getInetAddress() != null && !player.getName().equals(this.getName()))
					{
						
						final String ip = player.getClient().gco().getInetAddress().getHostAddress();
	...
	public void refreshOtherBoxes()
	{
		...
		if (getClient() != null && getClient().gco() != null && !getClient().gco().isClosed() && getClient().gco().getInetAddress() != null)
		{
			...
			for (final L2PcInstance player : players)
			{
				...
				{
					if (player.getClient() != null && player.getClient().gco() != null && !player.getClient().gco().isClosed() && !player.getName().equals(this.getName()))
					{
