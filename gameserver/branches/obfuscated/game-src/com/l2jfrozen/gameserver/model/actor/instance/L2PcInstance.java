...
public final class L2PcInstance extends L2PlayableInstance
{
	...
	public synchronized void rewardSkills(final boolean restore)
	{
		...
		if (lvl == 10)
		{
			...
			if (CommonConfig.DEBUG && skill != null)
			{
						...
						ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
						{
							...
							public void run()
							{
								...
								if (TvT._teams.indexOf(_teamNameTvT) != -1)
									...
									LOGGER.warn("DEBUGTVT: Player " + getName() + " got _teams -1 [3]. TeamName: " + _teamNameTvT);
						...
						ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
						{
							...
							public void run()
							{
								...
								if (TvT._teams.indexOf(_teamNameTvT) != -1)
									...
									LOGGER.warn("DEBUGTVT: Player " + getName() + " got _teams -1 [4].TeamName: " + _teamNameTvT);
						...
						ThreadPoolManager.getInstance().scheduleGeneral(new Runnable()
						{
							...
							public void run()
							{
								...
								if (CTF._teams.indexOf(_teamNameCTF) != -1)
									...
									LOGGER.warn("DEBUGCTF: Player " + getName() + " got _teams -1 [4]. TeamName: " + _teamNameCTF);
	...
	private boolean checkAntiFarm(final L2PcInstance targetPlayer)
	{
		...
		if (Config.ANTI_FARM_ENABLED)
		{
			...
			if (Config.ANTI_FARM_IP_ENABLED)
			{
				...
				{
					final String ip1 = this.getClient().getConnection().c().getHostAddress();
					final String ip2 = targetPlayer.getClient().getConnection().c().getHostAddress();
	...
	private void addItemReward(final L2PcInstance targetPlayer)
	{
		...
		{
			if (targetPlayer.getClient().getConnection().c() != getClient().getConnection().c())
			{
	...
	public void useMagic(final L2Skill skill, final boolean forceUse, final boolean dontMove)
	{
		...
		if (curr_skill_id != -1 && (isCastingNow() || isCastingPotionNow()))
		{
			...
			if (CommonConfig.DEBUG && getQueuedSkill() != null)
			{
	...
	public void startBotChecker()
	{
		if (_taskBotChecker == null)
		{
			...
			{
				LOGGER.warn("ATTENTION: Bot Checker is bad configured because " + FService.QUESTION_FILE + " has 0 words of 6 to 15 keys");
			}
		}
	}
	...
	public synchronized void deleteMe(final boolean disconnect)
	{
		if (Config.DEVELOPER)
		{
			if (getClient() != null && getClient().getConnection() != null && getClient().getConnection().c().getHostAddress() != null)
				LOGGER.info("Removing character: [" + getName() + "], account: [" + getAccountName() + "], ip: [" + getClient().getConnection().c().getHostAddress() + "]");
			else
	 ...
	 * friend.sendMessage("Friend: " + cha.getName() + " has logged off."); } } DatabaseUtils.close(rset); DatabaseUtils.close(statement); } catch(Exception e) { if(CommonConfig.ENABLE_ALL_EXCEPTIONS) e.printStackTrace(); LOGGER.warn("could not restore friend data:" + e); } finally {
	...
	public boolean checkMultiBox()
	{
		...
		if (getClient() != null && getClient().getConnection() != null && !getClient().getConnection().i() && getClient().getConnection().c() != null)
		{
			
			final String thisip = getClient().getConnection().c().getHostAddress();
			final Collection<L2PcInstance> allPlayers = L2World.getInstance().getAllPlayers();
			for (final L2PcInstance player : allPlayers)
			{
				if (player != null)
				{
					if (player.isOnline() == 1 && player.getClient() != null && player.getClient().getConnection() != null && !player.getClient().getConnection().i() && player.getClient().getConnection().c() != null && !player.getName().equals(this.getName()))
					{
						
						final String ip = player.getClient().getConnection().c().getHostAddress();
	...
	public void refreshOtherBoxes()
	{
		...
		if (getClient() != null && getClient().getConnection() != null && !this.getClient().getConnection().i() && getClient().getConnection().c() != null)
		{
			...
			for (final L2PcInstance player : players)
			{
				...
				{
					if (player.getClient() != null && player.getClient().getConnection() != null && !this.getClient().getConnection().i() && !player.getName().equals(this.getName()))
					{
